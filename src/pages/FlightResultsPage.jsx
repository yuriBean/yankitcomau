  ...
  const handleBookFlight = async (flightOffer) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-duffel-order', {
        body: { selectedOfferId: flightOffer.id },
      });

      if (error || data.error) throw new Error(error?.message || data.error);

      const duffelOrder = data.data;
      ...
      const bookingData = {
        ...
        total_price_ssp: flightOffer.price,
        payment_method: 'duffel_hosted_checkout',
        payment_status: 'pending_provider',
        booking_status: 'initiated_api',
        baggage_allowance_info: flightOffer.standardBaggage,
        duffel_order_id: duffelOrder.id,
      };
      ...
      if (payUrl) {
        ...
        toast({ title: "Order Created", description: `Order ${duffelOrder.id} confirmed.`, duration: 10000 });
        navigate(`/tracking/flight/${newBooking.id}`);
        onPromptBaggageListing(flightOffer);
      }
